import { generateProfessionalInvoicePDF, InvoiceData } from '../app/utils/pdf-generator-pro-v2';
import fs from 'fs';
...
async function main() {
  console.log('🧪 Testing PDF v2 with Korean fonts...\n');
  ...
  console.log('\n🔄 Generating PDF (locale: en, auto-detect Korean)...');
  const pdfBuffer = await generateProfessionalInvoicePDF(testData, 'en');
  
  const outputPath = path.join(process.cwd(), 'test-korean-invoice-v2.pdf');
